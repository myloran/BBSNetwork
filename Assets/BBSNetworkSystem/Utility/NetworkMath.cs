...
internal static class NetworkMathOld {
  ...
  public static int NativeToInteger(
      Type type, 
      object value, 
  ...
  public static object IntegerToNative(
      Type type, 
      object fieldValue, 
      int value, 
      int accuracy = DefaultAccuracy, 
    ...
    if (type == intType) {
      ...
      if (jumpThreshold != 0
        && math.greaterThan(
          math.abs(newValue - currentValue), 
          jumpThreshold)
      )
        return newValue;
...
internal abstract class NetworkMath {
  ...
  public abstract object IntegerToNative(
    object fieldValue,
    int oldValue,
    int newValue,
...
internal class NetworkMathInteger : NetworkMath {
  public override object IntegerToNative(
      object fieldValue, 
      int oldValue, 
      int newValue, 
      float deltaTimeFrame, 
...
internal class NetworkMathFloat : NetworkMath {
  const int DefaultAccuracy = 10;
  const int DefaultInterpolationSpeed = 1;
  const int DefaultJumpTheshold = 0;
  readonly int accuracy;
  readonly float interpolationSpeed;
  readonly float jumpThreshold;

  public NetworkMathFloat(
      int accuracy = DefaultAccuracy, 
      float interpolationSpeed = DefaultInterpolationSpeed, 
      float jumpThreshold = DefaultJumpTheshold
  ) {
  ...
  public override object IntegerToNative(
    ...
    var oldFloatValue = (float)oldValue / accuracy;
    var newFloatValue = (float)newValue / accuracy;
    var currentValue = (float)fieldValue;

    if (jumpThreshold != 0
      && math.greaterThan(
        math.abs(newFloatValue - currentValue), 
        jumpThreshold)
    )
    ...
    var totalDeltaTime = (currentValue - oldFloatValue)
      / (newFloatValue - oldFloatValue) * deltaTimeMessage;

    var lerpTime = ((totalDeltaTime + deltaTimeFrame)
      / deltaTimeMessage) * interpolationSpeed;
