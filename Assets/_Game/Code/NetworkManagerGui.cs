...
public class NetworkManagerGui : MonoBehaviour {
  ...
  void OnGUI() {
    GUILayout.Label(string.Format(
      "networkManager.IsConnected: {0}", 
      NetworkManager.Instance.IsConnected));

    GUILayout.Label(string.Format(
      "IsConnectedAndReady: {0}",
      NetworkManager.Instance.IsConnectedAndReady));
    ...
    GUILayout.Label(string.Format(
      "RoomsCount: {0}", 
      NetworkManager.Instance.RoomsCount));

    GUILayout.Label(string.Format(
      "PlayersInRoomCount: {0}", 
      NetworkManager.Instance.PlayersInRoomCount));

    GUILayout.Label(string.Format(
      "PlayersOnMasterServerCount: {0}", 
      NetworkManager.Instance.PlayersOnMasterServerCount));
    ...
    GUILayout.Label(string.Format(
      "Size of last send message: {0}", 
      NetworkManager.Instance.LastSendMessageSize));

    GUILayout.Label(string.Format(
      "Size of last received message: {0}", 
      NetworkManager.Instance.LastReceivedMessageSize));
